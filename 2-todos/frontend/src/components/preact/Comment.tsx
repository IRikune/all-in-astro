...
export function Comment({
  ...
  useSignalEffect(() => {
    ...
    useGetUser({userID: creator}).then((res) => {
      if (res.ok && res.data) {
        user.value = res.data;
      }
    });
  ...
  return (
    <article
      class={`flex flex-row mx-10 hover:*:opacity-100! hover:cursor-pointer ${className}`}
    >
      <aside>
      ...
      <main class="w-full">
        <header class="flex gap-3 items-center">
          <span class="font-medium text-sm">{user.peek()?.name}</span>
          <span class="text-xs text-neutral-700">{formatedDate}</span>
        </header>
        <p class="flex text-sm">{content}</p>
      </main>
