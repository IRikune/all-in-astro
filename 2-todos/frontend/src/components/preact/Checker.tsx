...
export function Checker({ priority, class: className }: Props) {
    ...
    return (
        <>
            ...
            >
                <input
                    class={`peer appearance-none size-4 cursor-pointer rounded-full flex align-middle z-10 items-center justify-center border-2 transition-colors duration-200 ${colors[priority]}`}
                    type="checkbox"
                    checked={checked}
                    onClick={() => { checked.value = true }}
                />

                <svg
                    class={`absolute left-[1px] top-[1px] peer-hover:opacity-100 opacity-0 transition-opacity size-3.5 peer-checked:motion-scale-in-140 ${iconColors[priority]}`}
                    viewBox="0 0 15 15"
                    fill="none"
