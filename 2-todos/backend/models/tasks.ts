...
export async function getManyTask({
	userID,
}: GetTasksOptions): Promise<Result<Task[]>> {
...
export async function getTask({
	taskID,
}: GetTaskOptions): Promise<Result<Task>> {
	if (!taskID) return { ok: false, data: null, message: "Task ID is required" };
...
export async function deleteTask({
	userID,
	taskID,
}: DeleteTaskOptions): Promise<Result<Task["id"]>> {
	if (!taskID) return { ok: false, data: null, message: "Task ID is required" };
...
export async function updateTask({
	taskID,
	newTask,
}: UpdateTaskOptions): Promise<Result<DenoKVCommit>> {
	if (!taskID) return { ok: false, data: null, message: "Task ID is required" };
	const key = ["tasks", taskID];
...
export async function deleteTasks({
	userID,
}: DeleteTasksOptions): Promise<Result<User["id"]>> {
