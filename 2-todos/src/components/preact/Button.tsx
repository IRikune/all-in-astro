...
export function Button({ to, page, children, class: className, icon, onclick }: Props) {
    ...
    return (
        <Tag href={to} class={`hover:bg-gray-200 flex transition-colors duration-200 rounded text-sm text-neutral-700 tracking-tight cursor-pointer items-center ${className} ${to != null && to === page && "bg-rose-100"}`} onClick={onclick} >
            {
                icon &&
